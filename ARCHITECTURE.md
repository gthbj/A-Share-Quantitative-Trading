...
| `strategy/ml_multi_horizon_picker/` | 实验包 | 多 Horizon ML 策略（PRD_20260524_05/06）：4 buy 模型 × horizon{1,5,10,20} + 1 sell 风险模型 + regime 三态调制 + 6 卖出触发；支持走步重训（walk_forward.py / model_registry.py）和可交易过滤（tradable.py，schema 与 BigQueryDataSource.trading_permissions 对齐）|
